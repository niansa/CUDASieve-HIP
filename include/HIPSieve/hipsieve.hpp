...
template <typename T>
inline void safeCudaFree(T * array) {if(array != NULL){hipFree(array); array = NULL;}}

template <typename T>
inline void safeCudaFreeHost(T * array) {if(array != NULL){hipHostFree(array); array = NULL;}}
...
inline T * safeCudaMalloc(T * d_a, size_t size)
{
  if(d_a == NULL){
    if(hipMalloc(&d_a, size) != hipSuccess){
      std::cerr << "CUDA device memory allocation error: CUDA API error " << hipMalloc(&d_a, size) << std::endl;
...
inline T * safeCudaMallocHost(T * h_a, size_t size)
{
  if(h_a == NULL){
    if(hipHostMalloc(&h_a, size) != hipSuccess){
      std::cerr << "CUDA host memory allocation error: CUDA API error " << hipMalloc(&h_a, size) << std::endl;
      exit(1);
    }
  }
  return (T *) h_a;
...
private:
  ...
  uint16_t gpuNum = 0;            // used with hipSetDevice
...
public:
  ...
  inline void setGpu(uint16_t gpuNum){this -> gpuNum = gpuNum; hipSetDevice(gpuNum);}
