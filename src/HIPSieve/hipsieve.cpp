...
{
  hipDeviceProp_t prop;
  hipGetDeviceProperties(&prop, gpuNum);
  return prop.name;
}
...
inline void CudaSieve::launchCtl()
{
  if(flags[0]){
    ...
    d_primeOut =  safeCudaMalloc(d_primeOut, numGuess*sizeof(uint64_t));
    hipMemset(d_primeOut, 0, numGuess*sizeof(uint64_t));
  }
...
inline void CudaSieve::getPrimes32()
{
  if(flags[0]){
    ...
    hipMemset(d_primeOut32, 0, numGuess*sizeof(uint32_t));
  }
...
void CudaSieve::copyAndPrint()
{
  ...
  hipMemcpy(h_primeOut, d_primeOut, count*sizeof(uint64_t), hipMemcpyDeviceToHost);
...
uint64_t * CudaSieve::getHostPrimes(uint64_t bottom, uint64_t top, size_t & count, uint16_t gpuNum)
{
  ...
  hipMemcpy(sieve->h_primeOut, sieve->d_primeOut, count*sizeof(uint64_t), hipMemcpyDeviceToHost);
...
std::vector<uint64_t> CudaSieve::getHostPrimesVector(uint64_t bottom, uint64_t top, size_t & count, uint16_t gpuNum)
{
  ...
  hipMemcpy(&temp[0], sieve->d_primeOut, count*sizeof(uint64_t), hipMemcpyDeviceToHost);
...
uint32_t * CudaSieve::getHostPrimes32(uint64_t bottom, uint64_t top, size_t & count, uint16_t gpuNum)
{
  ...
  hipMemcpy(sieve->h_primeOut32, sieve->d_primeOut32, count*sizeof(uint32_t), hipMemcpyDeviceToHost);
